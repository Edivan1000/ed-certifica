...
public class ClienteController {
    ...
    public String listarClientes(Model model) {
        ...
        for (Cliente cliente : clientes) {
            ...
            pedidosPorCliente.put(cliente.getId(), pedidos != null ? pedidos : new ArrayList<>()); // ✅ Evita valores
                                                                                                   // nulos
    ...
    public String salvarCliente(@ModelAttribute Cliente cliente) {
        ...
        if (isEdicao && clienteAnterior != null) {
            ...
            if (!detalhesEdicao.isEmpty()) {
                logAuditoriaService.registrarLog("Edição", "Cliente", cliente.getId(), "Administrador", detalhesEdicao);
            }
        } else {
    ...
    private String compararClientes(Cliente antigo, Cliente novo) {
        ...
        if (!Objects.equals(antigo.getNome(), novo.getNome())) {
            detalhes.add("🔹 Nome alterado: de '" + antigo.getNome() + "' para '" + novo.getNome() + "'.");
        }
        if (!Objects.equals(antigo.getEmail(), novo.getEmail())) {
            detalhes.add("🔹 Email alterado: de '" + antigo.getEmail() + "' para '" + novo.getEmail() + "'.");
        }
        if (!Objects.equals(antigo.getTelefone(), novo.getTelefone())) {
            detalhes.add("🔹 Telefone alterado: de '" + antigo.getTelefone() + "' para '" + novo.getTelefone() + "'.");
        }
        if (!Objects.equals(antigo.getCpfCnpj(), novo.getCpfCnpj())) {
            detalhes.add("🔹 CPF/CNPJ alterado: de '" + antigo.getCpfCnpj() + "' para '" + novo.getCpfCnpj() + "'.");
        }

        return String.join("\n", detalhes); // ✅ Agora sempre retorna algo se houver edição!
    }
